        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    TempData["OTP_Code"] = otp;
                    TempData["OTP_Email"] = user.Email;

                    // Gửi email OTP
                    await _emailSender.SendEmailAsync(user.Email, "Mã xác thực OTP",
                        $"Mã OTP để xác minh tài khoản của bạn là: <strong>{otp}</strong>");

                    // Điều hướng sang trang xác thực OTP
                    var verifyOtpUrl = Url.Action("VerifyOTP", "NguoiDung", new { area = "Admin", returnUrl });
                    return Redirect(verifyOtpUrl);
